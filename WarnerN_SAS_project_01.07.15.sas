...
DATE: Wednesday, January 07, 2015     TIME: 2:34:59 PM
PROJECT: WarnerN_SAS_project_01.07.15
PROJECT PATH: P:\QAC\qac200\students\nwarner\Assignments\WarnerN_SAS_project_01.07.15.egp
---------------------------------------- */
...
%LET _CLIENTPROJECTNAME='WarnerN_SAS_project_01.07.15.egp';
...
PROC SQL;
   ...
   SELECT t1.AMNURS12, 
          ...
          t1.AMNEXP12, 
          t1.EDUYRDEG
      FROM EC100014.meps_fullyr_2012 t1
      WHERE t1.AGE12X >= 18
...
/* -------------------------------------------------------------------
   ...
   Generated on: Wednesday, January 07, 2015 at 2:34:41 PM
...
PROC DATASETS NOLIST NODETAILS; 
   CONTENTS DATA=NATDATA.'NATVARRIABLESADULTS01.07.15'n OUT=WORK.SUCOUT1;

RUN;

DATA WORK.CONTContentsForNATVARRIABLESADUL(LABEL="Contents Details for NATVARRIABLESADULTS01.07.15");
   SET WORK.SUCOUT1;
RUN;
...
CREATE VIEW WORK.SCVIEW AS 
	SELECT DISTINCT memname LABEL="Table Name", 
			...
			typemem LABEL="Data Set Type" FROM WORK.CONTContentsForNATVARRIABLESADUL
	ORDER BY memname ; 

CREATE TABLE WORK.SCTABLE AS
	SELECT * FROM WORK.SCVIEW
		WHERE memname='NATVARRIABLESADULTS01.07.15';
QUIT;
...
PROC SORT DATA=WORK.CONTContentsForNATVARRIABLESADUL OUT=WORK.CONTContentsForNATVARRIABLESADUL;
   BY memname name;
RUN;
...
CREATE TABLE WORK.SCTABLE AS
	SELECT * FROM WORK.CONTContentsForNATVARRIABLESADUL
		WHERE memname='NATVARRIABLESADULTS01.07.15';
QUIT;
...
/* -------------------------------------------------------------------
   ...
   Generated on: Wednesday, January 07, 2015 at 2:34:44 PM
